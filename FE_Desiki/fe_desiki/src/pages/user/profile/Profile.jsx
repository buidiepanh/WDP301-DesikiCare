...
const Profile = () => {
  ...
  const handleCancelOrder = async (orderId) => {
    try {
      ...
      if (response) {
        ...
        toast.error(
          response.message || "Hủy đơn hàng thất bại, vui lòng thử lại!"
        );
      }
    } catch (error) {
  ...
  const getStatusColor = (statusId) => {
    switch (statusId) {
      case 1:
        return "orange"; // Chờ Xử Lí
      case 2:
        return "blue"; // Đang Giao
      case 3:
        return "green"; // Đã Giao
      case 4:
        return "red"; // Đã Hủy
      default:
        return "default";
          ...
          <div className="profile-right">
            ...
            <Table
              ...
              columns={[
                ...
                {
                  title: "Số Điểm Sử Dụng",
                  dataIndex: ["order", "pointUsed"],
                  key: "pointUsed",
                  width: 120,
                  render: (points) => (
                ...
                {
                  title: "Số Vé Thưởng",
                  dataIndex: ["order", "gameTicketReward"],
                  key: "gameTicketReward",
                  width: 120,
                  render: (tickets) => (
                ...
                },
                {
                  ...
                  render: (_, record) => (
                    ...
                    >
                      ...
                      {/* Nút Thanh Toán - chỉ hiển thị khi isPaid = false */}
                      {!record.order.isPaid && (
                        <Button
                          type="primary"
                          size="small"
                          ...
                          onClick={() => handleOrderPayment(record.order._id)}
                        >
                          Thanh toán
                        </Button>
                      )}
                      {/* Nút Hủy Đơn - chỉ hiển thị khi orderStatusId = 1 hoặc 2 và isP
aid = true */}
                      {(record.orderStatus._id === 1 ||
                        record.orderStatus._id === 2) &&
                        record.order.isPaid && (
                          <Popconfirm
                            title="Bạn có chắc chắn muốn hủy đơn hàng này?, đơn hàng
hủy do khách sẽ bị mất đi số điểm thưởng đã dùng cho đơn hàng đó!"
                            onConfirm={() =>
                              handleCancelOrder(record.order._id)
                            }
                            okText="Có"
                            cancelText="Không"
                          >
